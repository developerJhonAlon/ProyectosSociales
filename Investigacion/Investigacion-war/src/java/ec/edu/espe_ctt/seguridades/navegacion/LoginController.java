...
//        }
        ...
        try {
            ...
            if (usu.getUsuEstado()==null || usu.getUsuEstado().equalsIgnoreCase("I")) {
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Cuenta Inactiva"));
                return null;
            }
            BigDecimal idSistema = ParametrosGlobales.AMBIENTE_PRODUCCION1 ? new BigDecimal(3) : new BigDecimal(4);
            perfil = usuPerfilFacade.findPerfilesByUsuario(usu, idSistema);
            if (perfil == null) {
